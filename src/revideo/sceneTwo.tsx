...
export default makeScene2D('scene', function* (view) {
  ...
  const words = useScene().variables.get('words', [])() as WordType[];
  ...
  yield view.add(
    <>
      ...
      <Audio src={'https://revideo-example-assets.s3.amazonaws.com/chill-beat-2.mp3'} play={true} volume={0.1} />
    </>,
  );

  yield* all(displayImages(imageContainer, images, duration), displayWords(textContainer, words, textSettings));
});
...
function* displayWords(container: Reference<Layout>, words: WordType[], settings: captionSettings) {
